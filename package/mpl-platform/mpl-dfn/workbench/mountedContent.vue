<script setup lang="ts">
  ...
  const guildSpanList = ref<Array<number>>(
    Array.from({ length: 24 }).fill('').map((_, i) => i + 1)
  )
  const activeNode = ref<MPL.Node | null>(null)
  const nodeList = ref<MPL.Node[]>([]) // 平台缓存一份nodeList数据
      ...
      (e) => {
        ...
        if (obj.type === 'DRAG_START_ELEMENT') {
          ...
          if (dragStartNode.value) {
            const { w, h } = dragStartNode.value.pos.pc

            window.mplHighlightBlockSize = [`w-${w}`, `h-${h}`]
  ...
  function drop() {
    ...
    if (dragStartNode.value) {
      ...
      window.mplHighlightBlockSize = [
        `w-${activeNode.value.pos.pc.w}`,
        `h-${activeNode.value.pos.pc.h}`
      ]
...
<template>
  ...
  <div class="content-box">
    <div v-if="visibleGuildLine" class="dividing-rule--box">
      <div v-for="p in guildSpanList" :key="p">
        <span>{{ p }}</span>
      </div>
    </div>
    <div id="workbenchDragContainer" class="mounted-workbench-box mpl-scroll-none"
      ...
      <el-form status-icon class="mpl-root-form">
        <WorkbenchRootNode :node-list="mpl.api.formatNodeListToTree(nodeList)" ui="elementPlus" />
      </el-form>
    </div>
  </div>
</template>
