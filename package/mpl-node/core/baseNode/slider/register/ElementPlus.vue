...
import type { SliderFormProp } from '../define'
import SlotRenderNode from '../../../../components/SlotRenderNode.vue'

const el = defineModel<SliderFormProp>()
const props = defineProps<{ w: string | number, h: number | string }>()
</script>

<template>
  <template v-if="el">
    <el-form-item theme="mpl" :required="el.slider.required" class="mpl-form-item" :cid="el.cid" :prop="el.slider.prop"
      ...
      <div class="mpl-content" :class="[formContentPosClassName.get(el.slider.pos)]">
        slider
      </div>
      <template #error>
