...
import { newQrCodeNodeBox, QrCode } from './define'
import renderCode from './renderCode'
import type { QrCodeBox } from './define'
...
export default <SettingMap>{
  ...
  generator: {
    elementPlus: async (node: QrCodeBox) => (await import('./generator/ElementPlus')).default(node),
    vant4: async (node: QrCodeBox) => (await import('./generator/Vant4')).default(node),
    antDesign: async (node: QrCodeBox) => (await import('./generator/AntDesign')).default(node),
    antDesignMobile: async (node: QrCodeBox) => (await import('./generator/AntDesignMobile')).default(node),
    vars: async (node: QrCodeBox) => (await import('./generator/ref')).default(node)
  }
}
