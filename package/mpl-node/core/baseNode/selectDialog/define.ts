import type {
  ComponentBaseExport,
  FormItemSize,
  FormLabelProp,
  Node,
  NodePos,
  FormItemRule,
  EventNode,
} from "@mpl/typings";
import { SelectFormProp } from "index";

export interface SelectDialogNodeProp {
  model: any; // 服务绑定的接口选中值
  modelProp: {
    limit: number; //当选择数据存在多选时的存储值
  }
  size: "sm" | "m" | "l";
  width: number | ''; // '' 代表默认值
  required: boolean;
  pos: 'l' | 'c' | 'r';
  clearable: boolean; // 可清空
  rules: FormItemRule[]; // 校验规则列表
  readonly: boolean;
  placeholder: string;
}

function pos(): { pc: NodePos, mobile: NodePos } {
  return {
    pc: {
      w: 6,
      h: 4,
      minH: 5,
      maxH: 7,
      minW: 1,
      type: "w",
      mod: "formItem",
    },
    mobile: {
      w: 24,
      h: '76px',
      minH: 8,
      maxH: 20,
      minW: 24,
      type: "h",
      mod: "formItem",
    }
  };
}

export interface SelectDialogFormProp extends Node {
  label: FormLabelProp;
  size: FormItemSize;
  cid: string;
  selectDialog: SelectDialogNodeProp;
  pos: { pc: NodePos, mobile: NodePos }
}

export function newSelectDialogNode(cid: string): SelectDialogFormProp {
  return {
    mpl_group: "表单控件",
    mpl_title: "基础输入",
    mpl_zh: "输入",
    mpl_ce: "c",
    mpl_version: "1.0.0",
    cid,
    visible: true,
    tag: "mpl-select-dialog",
    size: "small",
    childIds: [],
    classList: [],
    userClassName: [],
    style: "",

    label: {
      show: true,
      width: 120,
      text: "基础输入框",
      pos: "r",
      icon: "icon-info2",
      iconTheme: "#C3002F",
      iconText: "基础信息",
    },
    selectDialog: {
      required: false,
      modelProp: {
        limit: 1
      },
      model: "",
      width: "",
      pos: "l",
      size: 'sm',
      clearable: true,
      readonly: false,
      placeholder: "请输入",
      rules: []
    },
    events: [],
    pos: pos(),
  }
}

function getTemplateCode(node: SelectDialogFormProp): string {
  let eventStr = "";

  if (node.events.includes("input")) {
    eventStr += ` @input="mpl.${node.cid}.input"`;
  }
  if (node.events.includes("change")) {
    eventStr += ` @change="mpl.${node.cid}.change"`;
  }
  if (node.events.includes("blur")) {
    eventStr += ` @blur="mpl.${node.cid}.blur"`;
  }
  if (node.events.includes("focus")) {
    eventStr += ` @focus="mpl.${node.cid}.focus"`;
  }
  if (node.events.includes("disabled")) {
    eventStr += ` :disabled="mpl.${node.cid}.disabled"`;
  }
  if (node.events.includes("readonly")) {
    eventStr += ` :readonly="mpl.${node.cid}.readonly"`;
  }
  if (node.events.includes("visible")) {
    eventStr += ` :visible="mpl.${node.cid}.visible"`;
  }

  if (node.events.includes("clearable")) {
    eventStr += ` :clearable="mpl.${node.cid}.clearable"`;
  }

  if (node.events.includes("placeholder")) {
    eventStr += ` :placeholder="mpl.${node.cid}.placeholder"`;
  }

  return `
		<!-- 1、基础输入框 template 节点源码 -->
    <!-- 2、DOM只占一行，编辑template时，一次只能编辑一行。用户选择编辑的行【即DOM】，MPL展示该DOM的所有属性，用户选择属性或新增属性进行编辑更新。 -->
    <!-- 3、用户选择属性【属性需要根据MPL规则展示可编辑的范围，更换绑定、变量替换、禁止编辑、删除属性等】 -->
    <!-- 4、新增属性【查看是否存在冲突、key-value合法性】-->

		<mpl-select-dialog
      ${eventStr}
      :placeholder="mpl.${node.cid}.placeholder"
      :clearable="mpl.${node.cid}.clearable"
    />
	`;
}

// 向外部提供的API列表
function getNodeMethod() {
  return [
    { label: 'validate()', value: '校验组件' },
    { label: 'resetValidate()', value: '重置选择' },
    { label: 'required(true | false)', value: '设置是否必填' },
    { label: 'hidden()', value: '隐藏' },
    { label: 'show()', value: '显示' },
    { label: 'triggerVisible(true | false)', value: '反转显示或隐藏' },
  ]
}

// 向外部提供的可直接赋值的变量
function getNodeVar(cid: string): string {
  return `
    // 基础输入框组件变量
    const mplVar = {
      ${cid}: {
        visible: true, // 显示控件
        vmodel: null, // 选中值
        required: false, // 必填值
        disabled: false // 禁用变量
      }
    }
  `;
}

// 组件提供的事件绑定
const events: EventNode[] = [
  {
    name: "input",
    desc: "输入时",
    code: `console.log(value)`,
    type: "flow",
    doc: "/docs/event-input.md",
  },
  { name: "focus", desc: "聚焦时", code: "", type: "flow" },
  { name: "blur", desc: "失焦时", code: `console.log(value)`, type: "flow" },
  { name: "change", desc: "输入内容改变时", code: "", type: "flow" },
  { name: "clear", desc: "点击清空图标", code: "", type: "flow" },
]

export const selectDialogNode:ComponentBaseExport = {
  tag: 'mpl-select-dialog',
  comp: newSelectDialogNode,
  pos: pos(),
  events,
  getContextmenu: (node: SelectFormProp) => {
    return []
  },
  getTemplateCode,
  getNodeVar,
}