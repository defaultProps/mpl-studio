...
import { type RadioBoxFormProp, type RadioBoxNode, getDefaultRadioOptions } from '../define'

const props = defineProps<{ node: RadioBoxFormProp }>()
const radio = ref<RadioBoxNode>(props.node.radio)
...
<template>
  <el-form-item theme="mpl" :required="radio.required" class="mpl-form-item" :cid="props.node.cid"
    ...
    <div class="mpl-content" :class="[formContentPosClassName.get(radio.pos)]">
      <el-radio-group v-model="radioValue" class="ml-4">
        <template v-for="p in getDefaultRadioOptions()" :key="p.value">
          <el-radio v-if="radio.service.mode === 'static'" :value="p.value">
            {{ p.label }}
          </el-radio>
