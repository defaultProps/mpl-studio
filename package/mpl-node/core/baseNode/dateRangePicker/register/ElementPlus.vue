...
const el = defineModel<DateRangePickerFormProp>()
const props = defineProps<{ w: string | number, h: number | string }>()
</script>

<template>
  <template v-if="el">
    <el-form-item theme="mpl" :required="el.dateRangePicker.required" class="mpl-form-item" :cid="el.cid"
      :prop="el.dateRangePicker.prop"
      ...
      <div class="mpl-content" :class="[formContentPosClassName.get(el.dateRangePicker.pos)]">
        <el-date-picker v-model="el.dateRangePicker.model" type="datetimerange" start-placeholder="开始日期"
