...
const el = defineModel<ColorPickerFormProp>()
const props = defineProps<{ w: string | number, h: number | string }>()
</script>

<template>
  <template v-if="el">
    <el-form-item theme="mpl" :required="el.colorPicker.required" class="mpl-form-item" :cid="el.cid"
      :prop="el.colorPicker.prop"
      ...
      <div class="mpl-content" :class="[formContentPosClassName.get(el.colorPicker.pos)]">
        <SlotRenderNode :nodes="el.slotNodes.filter(v => v.pos === 'l')" />
        <el-color-picker v-model="el.colorPicker.model" />
