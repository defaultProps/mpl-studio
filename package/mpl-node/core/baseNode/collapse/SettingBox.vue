...
function removeNode(item: CollapseItem, i: number) {
  if (item.mpl_children.length) {
    ElMessageBox.confirm('该面板下存在子节点，确定删除该面板吗？', '提示', {
      ...
      activeNode.value.itemList.splice(i, 1)
    }).catch(() => {
      // 取消删除
    })
  } else {
    ...
    <VueDraggable v-if="activeNode.itemList.length" v-model="activeNode.itemList" :animation="300"
      ...
      <div v-for="(item, i) in activeNode.itemList" :key="i" class="slot-collapse-line">
        <button type="button" class="mpl-btn mr-5 icon icon-edit5"
          :class="{ 'is-active': selectCollapse?.value === item.value }" @click="handleSelectCollapse(item)" />
