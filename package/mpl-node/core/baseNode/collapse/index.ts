...
import { collapse, newCollapseNode as newNode, type CollapseProp } from './define'
...
export default <SettingMap>{
  ...
  generator: {
    elementPlus: async (node: CollapseProp) => (await import('./generator/ElementPlus')).default(node),
    vant4: async (node: CollapseProp) => (await import('./generator/Vant4')).default(node),
    antDesign: async (node: CollapseProp) => (await import('./generator/AntDesign')).default(node),
    antDesignMobile: async (node: CollapseProp) => (await import('./generator/AntDesignMobile')).default(node),
    vars: async (node: CollapseProp) => (await import('./generator/ref')).default(node)
  }
}
