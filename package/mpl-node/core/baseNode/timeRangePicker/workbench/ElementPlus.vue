...
<template>
  <el-form-item theme="mpl" :required="mplSwitch.required" class="mpl-form-item" :cid="props.node.cid"
    ...
    <div class="mpl-content" :class="[formContentPosClassName.get(mplSwitch.pos)]">
      <SlotRenderNode :nodes="props.node.slotNodes.filter(v => v.slotPos === 'l')" />
      <el-time-picker v-model="timeRangePickerValue" is-range mpl range-separator="至" start-placeholder="开始时间"
        end-placeholder="结束时间" />
      <SlotRenderNode :nodes="props.node.slotNodes.filter(v => v.slotPos === 'r')" />
    </div>
  </el-form-item>
</template>
