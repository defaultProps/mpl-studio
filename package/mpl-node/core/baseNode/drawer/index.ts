...
import { drawer, newDrawerNode, type DrawerProp } from './define'
...
export default <SettingMap>{
  ...
  generator: {
    elementPlus: async (node: DrawerProp) => (await import('./generator/ElementPlus')).default(node),
    vant4: async (node: DrawerProp) => (await import('./generator/Vant4')).default(node),
    antDesign: async (node: DrawerProp) => (await import('./generator/AntDesign')).default(node),
    antDesignMobile: async (node: DrawerProp) => (await import('./generator/AntDesignMobile')).default(node),
    vars: async (node: DrawerProp) => (await import('./generator/ref')).default(node)
  }
}
