...
import { baseBox, newBaseBox, type BaseBoxProp } from './define'
...
export default <SettingMap>{
  ...
  generator: {
    elementPlus: async (node: BaseBoxProp) => (await import('./generator/ElementPlus')).default(node),
    vant4: async (node: BaseBoxProp) => (await import('./generator/Vant4')).default(node),
    antDesign: async (node: BaseBoxProp) => (await import('./generator/AntDesign')).default(node),
    antDesignMobile: async (node: BaseBoxProp) => (await import('./generator/AntDesignMobile')).default(node),
    vars: async (node: BaseBoxProp) => (await import('./generator/ref')).default(node)
  }
}
