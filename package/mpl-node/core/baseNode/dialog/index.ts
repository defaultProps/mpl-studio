...
import { dialog, newDialogNode, type DialogProp } from './define'
...
export default <SettingMap>{
  ...
  generator: {
    elementPlus: async (node: DialogProp) => (await import('./generator/ElementPlus')).default(node),
    vant4: async (node: DialogProp) => (await import('./generator/Vant4')).default(node),
    antDesign: async (node: DialogProp) => (await import('./generator/AntDesign')).default(node),
    antDesignMobile: async (node: DialogProp) => (await import('./generator/AntDesignMobile')).default(node),
    vars: async (node: DialogProp) => (await import('./generator/ref')).default(node)
  }
}
