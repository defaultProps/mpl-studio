...
const el = defineModel<SwitchFormProp>()
const props = defineProps<{ w: string | number, h: number | string }>()
</script>

<template>
  <template v-if="el">
    <el-form-item theme="mpl" :required="el.switch.required" class="mpl-form-item" :cid="el.cid" :prop="el.switch.prop"
      ...
      <div class="mpl-content" :class="[formContentPosClassName.get(el.switch.pos)]">
        <el-switch v-model="el.switch.model" />
      </div>
      <template #error>
